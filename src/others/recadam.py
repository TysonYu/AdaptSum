...
class RecAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-6, weight_decay=0.0, correct_bias=True, anneal_fun='sigmoid', anneal_k=0, anneal_t0=0, anneal_w=1.0, pretrain_cof=5000.0, pretrain_params=None):
        if lr < 0.0:
        ...
        print("anneal_k: {:4f}, anneal_t0: {:4f}".format(anneal_k, anneal_t0))
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, correct_bias=correct_bias, anneal_fun=anneal_fun, anneal_k=anneal_k, anneal_t0=anneal_t0, anneal_w=anneal_w, pretrain_cof=pretrain_cof, pretrain_params=pretrain_params)
        ...
        for group in self.param_groups:
            for p, pp in zip(group["params"], group["pretrain_params"]):
                ...
                if group['anneal_w'] > 0.0:
                    # print("anneal_fun: {}, anneal_w: {:4f}, anneal_k: {:4f}, anneal_t0: {:4f}".format(group['anneal_fun'], group['anneal_w'], group['anneal_k'], group['anneal_t0']))
                    # We calculate the lambda as the annealing function
                    anneal_lambda = anneal_function(group['anneal_fun'], state["step"], group['anneal_k'], group['anneal_t0'], group['anneal_w'])
                    assert anneal_lambda <= group['anneal_w']
                    # print(anneal_lambda, step_size)
